...
import com.kkSpringBoot2.oauth2Common.common.MyOAuth2AccessToken;
...
public class MyTokenServices extends DefaultTokenServices {
    ...
    private int accessTokenValiditySeconds = 20 ; // default 12 hours.60 * 60 * 12

    private boolean supportRefreshToken = false;

    private boolean reuseRefreshToken = true;

    private EMyTokenStore tokenStore;
    ...
    public OAuth2Authentication loadAuthentication(String accessTokenValue) throws AuthenticationException,
        ...
        String refreshTokenValue = tokenStore.readRefreshTokenValueByAccessToken(accessTokenValue);
        if (accessToken == null) {
            throw new InvalidTokenException("Invalid access token: " + accessTokenValue);
        }
        else if (accessToken.isExpired()) {
            // 进行续签操作 ，失败就清空 -- accessToken 和 RefreshToken

            if (!supportRefreshToken) {
                throw new InvalidGrantException("Invalid refresh token: " + refreshTokenValue);
            }

            OAuth2RefreshToken refreshToken = tokenStore.readRefreshTokenByTokenID(refreshTokenValue);
            if (refreshToken == null) {
                throw new InvalidGrantException("Invalid refresh token: " + refreshTokenValue);
            }
           ...
           String clientId = authentication.getOAuth2Request().getClientId();
            ...
            accessToken = (MyOAuth2AccessToken) createAccessToken(authentication, refreshToken);
            tokenStore.storeAccessToken(accessToken, authentication);
            if (!reuseRefreshToken) {
                tokenStore.storeRefreshToken(accessToken.getRefreshToken(), authentication);
            }

            //  ---------------- 要么实现上面的 续签操作， 要么打开下面注释 ，直接清除操作 -----------------------
            /*
            // 自定义 把 refreshToken 删除
            tokenStore.removeRefreshTokenByAccessToken(accessToken);
            tokenStore.removeAccessToken(accessToken);
            throw new InvalidTokenException("Access token expired: " + accessTokenValue);*/
        }

        OAuth2Authentication result = tokenStore.readAuthentication(accessToken);
        result.setDetails(accessToken);
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        MyOAuth2AccessToken token = new MyOAuth2AccessToken(UUID.randomUUID().toString());
        ...
        token.setMy_value("只是我自定义的数据111");
    ...
    public void setTokenStore(TokenStore tokenStore) {
        this.tokenStore = (EMyTokenStore) tokenStore;
    }
