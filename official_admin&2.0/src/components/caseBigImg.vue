  ...
  <div class="add" :style="{height:mHeight}">
    ...
    <el-form ref="form" :model="form" label-width="100px">
      <el-form-item label="案例标题">
        <el-input v-model="form.subhead"  placeholder="输入20个字符以内" clearable></el-input>
      </el-form-item>
      <el-form-item label="大图说明">
        <el-input v-model="form.desc" placeholder="输入20个字符以内" clearable></el-input>
      </el-form-item>
      <el-form-item label="图片网络地址">
        ...
        <el-form-item label="更换图片">
              <el-upload
                class="upload-demo"
                ref="upload"
                action="/api/successCase/updateSuccessCaseBigImg"
                :file-list="fileList"
        ...
        methods: {
          ...
          submitUpload(){
            if(this.fileList.length == 0){
              https.fetchPost("/api/successCase/updateSuccessCaseBigImg",this.form).then((data) => {
         ...
         onSuccess(response, file, fileList){
          ...
          onError(err, file, fileList){
            if(err.message == 'Request failed with status code 401' || err.status == "40100"){
