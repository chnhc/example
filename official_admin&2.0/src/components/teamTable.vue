    ...
    <el-table
       ...
       <el-table-column
         ...
         <template slot-scope="scope">
           ...
           <el-button type="text" size="small" disabled v-if="scope.row.team_status == 1">替换</el-button>
     ...
     <div class="block">
      ...
      <el-dialog title="替换操作" :visible.sync="dialogFormVisible" @close="close">
        <el-form :model="form">
          <el-form-item label="替换团队项" >
            <el-select v-model="form.selectID" placeholder="请选择团队项" style="width:80%">
              <el-option v-for="(item, index) in showData" :key="index" :label="item.teamTitle" :value="item.teamID"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
      ...
      <el-dialog title="修改操作" :visible.sync="dialogForm1Visible" @close="close">
        <el-form ref="form1" :model="form1" label-width="100px">
          <el-form-item >
              <span>说明：新添加项不会直接生效，需要到团队表中，“手动-替换” 为页面展示项</span>
          </el-form-item>
            <el-form-item label="团队标题">
              <el-input v-model="form1.team_title" placeholder="输入20个字符以内" clearable></el-input>
            </el-form-item>
            <el-form-item label="团队说明">
              <el-input v-model="form1.team_desc"  placeholder="输入20个字符以内" clearable></el-input>
            </el-form-item>
            ...
            <el-form-item label="更换图片">
                  <el-upload
                    class="upload-demo"
                    ref="upload"
                    action="/api/aboutUS/updateTeamByID"
                    :file-list="fileList"
                    :limit="1"
                    :headers="getHeader"
  ...
  export default {
    ...
    data () {
        return {
          ...
          select_id : '',
          showData : [],
          check: ['团队编号','团队图片','团队标题','团队说明','状态','日期'],
          pageNum : 1,
          pageSize : 5,
          total: 0,
          vertical: 'apple',
    ...
    methods:{
      deletedata(row){
        ...
        https.fetchPost("/api/aboutUS/changeTeam",this.form2).then((data) => {
      ...
      addFile(file,fileList){
       ...
       submitUpload(){
         if(this.fileList.length == 0){
           https.fetchPost("/api/aboutUS/updateTeamByID",this.form1).then((data) => {
                         if(data.data.status != 20000){
                           this.onError(data.data)
                         }else{
                           this.getData()
      ...
      confirm(){
        https.fetchPost("/api/aboutUS/updateTeam",{'old_team_id':this.form.selectID,'tra_team_id':this.select_id}).then((data) => {
                      if(data.data.status != 20000){
                        this.onError(data.data)
                      }else{
                        this.success()
      ...
      tran(row){
        this.select_id = row.team_id
        this.dialogFormVisible = true
        https.fetchGet("/api/aboutUS/getTeamShow").then((data) => {
      ...
      getData(){
        https.fetchGet("/api/aboutUS/getTeam",{'pageNum': this.pageNum, 'pageSize': this.pageSize}).then((data) => {
